<template>
  <div>
    <v-row >
      <v-col md4>
        <v-btn router :to="{name: 'create', params: {resource}}" color='primary'>{{$t('Create')}}</v-btn>
      </v-col>
      <v-col md8>
    ...
    <v-card >
      ...
      <template slot="items" slot-scope="props">
        ...
        <td v-if="actions" width="180">
          <v-btn v-if="button && button.icon" v-for="(button, key) in actions" :key="key" router floating small dark :to="{name: 'action', params: {resource, id:props.item.id, action: key}}" :color="button.type == 'primary' || button.type == ''" :success="button.type == 'success'">
            <v-icon>{{button.icon}}</v-icon>
          </v-btn>
  
          <v-btn v-if="actions === true || actions.edit === true" router color='primary' floating small dark :to="{name: 'edit', params: {resource,id:props.item.id}}">
            <v-icon>edit</v-icon>
          </v-btn>
